...
router.post('/chat', async (req, res) => {
  const { message, userId, role = 'employee' } = req.body;
...
${myTasks.map(t => `- "${t.title}" (Status: ${t.status}, Priority: ${t.priority}, Due: ${t.dueDate})`).join('\n')}`;
  ...
  const geminiReply = await generateGeminiContent(message, systemContext, requestKey);

  if (geminiReply) {
    return res.json({ reply: geminiReply, provider: 'Google Gemini AI' });
  }

  // Smart Fallback Queries (if Gemini API experiences a temporary 503 high demand spike)
  ...
  if (lowerMsg.includes('next task') || lowerMsg.includes('what should i work on') || lowerMsg.includes('suggest')) {
    ...
    if (pendingTasks.length === 0) {
      ...
      const isOverdue = topTask.dueDate < todayStr;
      
      reply = `Recommended Next Task:\n\nTask: "${topTask.title}"\n` +
        `- Priority: ${topTask.priority}\n` +
        `- Status: ${topTask.status}\n` +
        `- Due Date: ${topTask.dueDate} ${isOverdue ? '(Overdue)' : ''}\n` +
  ...
  if (lowerMsg.includes('team update') || lowerMsg.includes('team summary') || lowerMsg.includes('team progress') || lowerMsg.includes('manager summary')) {
    ...
    const overdueTasks = allTasks.filter(t => t.dueDate < todayStr && t.status !== 'Done');
    ...
    const userBreakdown = db.users.filter(u => u.role === 'employee').map(u => {
      ...
      const userOverdue = userAssigned.filter(t => t.dueDate < todayStr && t.status !== 'Done').length;
